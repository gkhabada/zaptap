            ...
            form.form(@submit.prevent="handleSubmit")
              ...
              template(v-for="(phone, i) in phones")
                label.phone
                  ...
                  .remove__phone(@click="removePhone(i)" v-else)
...
export default {
  ...
  computed: {
    invalidForm() {
      ...
      // if (this.transport.length === 0) return true;
  ...
  methods: {
    async handleSubmit() {
      ...
      const userData = {
        ...
        receiver_trans_comps: this.getStringFromArray(this.transport),
    ...
    async getUser() {
      ...
      const getData = async () => {
        if (this.tempUser) {
          return { data: this.tempUser };
        } else {
          return post("/profile/get-user-profile/" + id).catch(() => {
            return { data: null };
          });
        }
      };
      ...
      this.transport = this.getArrayFromString(data.receiver_trans_comps);
